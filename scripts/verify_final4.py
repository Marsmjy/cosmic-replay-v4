...
# 找0512 - 选最短文件名
candidates = sorted([f for f in os.listdir(HAR_DIR) if "0512" in f and f.endswith(".har")], key=len)
if candidates:
    HARS.append((candidates[0], "新增入职0512测试"))
    print(f"0512: {candidates[0]}")
...
# 汇总
print(f"\n\n{'='*50}\n最终结果\n{'='*50}")
pass_count = sum(1 for _, s, _, _ in results if s == "passed")
